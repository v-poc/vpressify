...
/**
 * Type helper to make it easier to create vpressify config
 */
...
function resolveInVpressify(root: string, targetPath: string) {
  return path.resolve(root, ".vpressify", targetPath);
}

export function resolveThemePath(
  root: string
): [themePath: string, useDefaultTheme: boolean] {
  const userThemePath = resolveInVpressify(root, "theme");
...
export async function loadUserConfig(root: string): Promise<{
  ...
  for (const configFile of POSSIBLE_CONFIG_FILES) {
    const tryPath = resolveInVpressify(root, configFile);
...
function resolveTailwindConfig(
  ...
  const content = Object.values(pages)
    ...
    .concat(
      resolveInVpressify(root, "**/*.{js,jsx,ts,tsx}"),
