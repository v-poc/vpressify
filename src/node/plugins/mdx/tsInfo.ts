...
export function extractInterfaceInfo(
  ...
  if (!fileSymbol || !fileSymbol.exports) {
    throw new Error(`[vpressify] Unexpected fileSymbol. ${filePath}`);
  }
  ...
  if (!exportSymbol) {
    throw new Error(
      `[vpressify] Named export '${exportName}' is not found in file ${filePath}`
    );
  }
  ...
  if (!sourceDeclaration) {
    throw new Error(
      `[vpressify] Can not find sourceDeclaration for ${exportName}`
    );
  }
  ...
  function collectInterfaceInfo(
    ...
    if (!ts.isInterfaceDeclaration(sourceDeclaration)) {
      throw new Error(`[vpressify] Target is not an InterfaceDeclaration`);
    }

    if (!sourceSymbol) {
      throw new Error(`[vpressify] Can not find symbol`);
    }
    ...
    sourceSymbol.members?.forEach((member) => {
      ...
      if (
        ...
        throw new Error(
          `[vpressify] Unexpected declaration type in interface. name: ${memberName}, kind: ${
